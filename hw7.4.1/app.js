...
//console.log(products.phones[0].price);


// Почему не работает обращание к значениям объектов через .
// Приходится ставить []
function showCategory(category) {
    let markUp = '';
    for (let i = 0; i < products[category].length; i++) {
        markUp = getProductMarkup(products[category][i]);
        productsEl.insertAdjacentHTML('afterbegin', markUp);
    }
}
